	...
	class customer 
	{
		...
		public function insert_customer($data){
			...
			if($name=="" || $city=="" || $zipcode=="" || $email=="" || $address=="" || $country=="" || $phone=="" || $password==""){
				...
				if($result_check){
					$alert = "<span class='success'>Email Already Existed</span>";
					return $alert;
				}else{
					...
					if($result){
						...
						$alert ="<span class='success'>Customer Created Not Successfully</span>";
						return $alert;
					}
		...
		public function login_customer($data){
			...
			$password = mysqli_real_escape_string($this->db->link, md5($data['password']));
			if($email=='' || $password==''){
				$alert = "<span class='success'>Password and email must be not empty</span>";
				return $alert;
			}else{
				$check_login = "SELECT * FROM tbl_customer WHERE email='$email' AND password='$password'";
				$result_check = $this->db->select($check_login);
				if($result_check != fales){
					...
					$alert = "<span class='success'>Email And Password doesn't match</span>";
					return $alert;
				}
			}
		}
			...
			if($name=="" || $zipcode=="" || $email=="" || $address=="" || $phone==""){
				...
				return $alert;
					...
					if($result){
						...
						$alert ="<span class='success'>Customer Update Not Successfully</span>";
						return $alert;
					}
